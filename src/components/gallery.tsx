...
interface GalleryProductProps {
  hoveredColor: { src: string } | null // Accept hovered color image src
              ...
              <Card className="relative w-full rounded-none">
                <CardContent className="flex items-center justify-center p-0">
                  ...
                  <Image
                    src={
                      status && hoveredColor
                        ? hoveredColor.startsWith("http") // Nếu đã là URL đầy đủ, giữ nguyên
                          ? hoveredColor
                          : `${process.env.NEXT_PUBLIC_API_URL_IMAGE}${hoveredColor}`
        ...
        {images?.map((items, i) => (
          <button key={i} onClick={() => api?.scrollTo(i)}>
            <Image
              ...
              width={100}
              height={90}
              className={`h-full border-2 object-cover transition-all md:w-[115px] ${
                i === current
                  ? "border-[#79BCCF]"
